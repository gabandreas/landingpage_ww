    ...
    <section ref={sectionRef} id="home" className="relative w-full overflow-hidden bg-[#040714] min-h-[100dvh] flex flex-col">
      ...
      <div className="absolute inset-0 overflow-hidden opacity-100 pointer-events-none">
         ...
         <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6 p-4 h-[150%] -mt-20 transform -rotate-6 scale-105 origin-center">
               ...
               >
                  {[...column1, ...column1, ...column1].map((src, i) => (
                    <div key={i} className="relative w-full aspect-[2/3] rounded-lg overflow-hidden shadow-lg">
                       <Image src={src} alt="Poster" fill className="object-cover" sizes="(max-width: 768px) 50vw, 20vw" />
                    </div>
                  ))}
               </motion.div>
            </div>
            ...
            {/* Kolom 3 (Hidden on smallest mobile) */}
            <div className="flex flex-col gap-6 hidden sm:flex">
             ...
             {/* Kolom 4 (Hidden on mobile/tablet) */}
      ...
      {/* === 2. NAVBAR === */}
      <div className="fixed top-0 left-0 w-full z-50 flex justify-between items-center p-4 sm:p-6 pointer-events-none">
        <div className="h-7 w-24 sm:h-8 sm:w-28 relative pointer-events-auto opacity-90 hover:opacity-100 transition-opacity">
      ...
      <div className="relative z-20 w-full max-w-5xl mx-auto flex-grow flex flex-col justify-center items-center px-4 text-center pt-20 pb-10 sm:py-0">
        ...
        >
           ...
           {/* Store Buttons: Stack on mobile, row on desktop */}
